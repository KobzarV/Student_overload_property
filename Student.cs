namespace Studentns
{
    public class Student
    {
        ...
        public void Info()
        {
            ...
            Console.WriteLine($"Оцінкa за домашні роботи: {Math.Round(homeworks.Average() , 2)}");
            Console.WriteLine($"Оцінкa за підсумкові роботи: {Math.Round(FinalWorks.Average(), 2)}");
            Console.WriteLine($"Оцінкa за екзамени: {Math.Round(Exams.Average(), 2)}\n\n");
        }
        ...
        public static bool operator ==(Student st1, Student st2)
        {
            if (ReferenceEquals(st1, st2))
                return true;
            if (ReferenceEquals(st1, null) || ReferenceEquals(st2, null))
                return false;
            return st1.Homeworks.Average() == st2.Homeworks.Average();
        }
        ...
        {
            if (student.Homeworks.Average() >= 70) { return true; }
            else { return false; }
        }
    }
